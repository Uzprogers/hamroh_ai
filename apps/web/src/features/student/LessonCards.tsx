    ...
    <div className="grid gap-3 sm:grid-cols-2">
      {shown.map((lesson) => (
        <Link
          key={lesson.id}
          to={`/lesson/${lesson.id}`}
          className="group rounded-2xl border border-edge/70 bg-panel/50 p-4 transition hover:-translate-y-0.5 hover:border-teal/50"
        >
          <div className="flex items-start gap-3">
            <span className="grid h-9 w-9 shrink-0 place-items-center rounded-xl border border-edge bg-ink/30 text-teal">
